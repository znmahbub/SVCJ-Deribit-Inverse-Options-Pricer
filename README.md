...
5. **Calibrate model parameters** – Solve a weighted nonlinear least‑squares problem on the training set for each model.  Parameters are transformed to enforce positivity and correlation bounds, and soft penalties enforce the Feller condition and moment stability for jump models.
...
│   └── __init__.py                 # Exposes public API (BatchConfig, run functions)
├── data/
...
├── calibration_results.xlsx        # (generated) workbook storing calibration outputs
...
2. Collect option data with `python src/collect_deribit_snapshot.py --currency BTC` or use existing CSV files in `data/`.
3. Run the batch calibration via the notebook `notebooks/calibrate_all_to_excel.ipynb` or programmatically:

   ```python
   from pathlib import Path
   from src import BatchConfig, run_all_snapshots_to_excel

   cfg = BatchConfig(project_root=Path('.'),
                     output_xlsx=Path('calibration_results.xlsx'),
                     n_workers=4,
                     verbose=1)
   run_all_snapshots_to_excel(cfg)
   ```

4. Inspect the generated `calibration_results.xlsx` to see parameter time series and priced option data across all processed snapshots.

---

## License

This code is provided for research and educational purposes under the MIT license.  Contributions are welcome; please update the documentation in `docs/code_documentation.md` if you add new modules or functions.