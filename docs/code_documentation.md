...
* **Calibration** – `calibrate_model(df, model, ..., initial_params, bounds, penalty_coin, constraint_penalty, feller_eps, svcj_moment_eps, l2_prev_strength, previous_params, max_nfev, verbose, ...)` solves a **weighted nonlinear least‑squares** problem for the specified model. The objective is implemented as a stacked residual vector: data-fit residuals `w_i * (P_model_i - P_mkt_i)`, soft-constraint residuals for the Heston Feller condition (`sigma_v^2 ≤ 2 κ θ`) and SVCJ moment stability (`1 - ell_v * rho_j ≥ ε`), plus an optional L2 warm-start term `sqrt(l2_prev_strength) * (x - x_prev)` in transformed-parameter space. The L2 term is active only when `l2_prev_strength > 0` and `previous_params` is provided. The parameter vector is transformed (log for positive parameters, arctanh for correlations). `scipy.optimize.least_squares` is used under the hood and returns a `CalibrationResult` with parameter estimates, fit metrics and success flags.
