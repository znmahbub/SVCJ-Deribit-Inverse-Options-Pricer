...
def load_perp_history(data_dir: Path, filename: str | None = None) -> tuple[pd.DataFrame, str]:
    ...
    df = pd.read_csv(path)
    if {"currency", "timestamp", "close_price"} - set(df.columns):
        raise ValueError(f"{path} must contain currency, timestamp, close_price")
    df["currency"] = df["currency"].astype(str).str.upper()
    df["timestamp"] = pd.to_datetime(df["timestamp"], utc=True, errors="coerce")
    df["close_price"] = pd.to_numeric(df["close_price"], errors="coerce")
    return df.dropna(subset=["timestamp", "close_price"]).sort_values(["currency", "timestamp"]).reset_index(drop=True), used
